...
'''# Open data (change this when we have classes)
ds = xr.open_dataset('Data/Example_Data/e-obs_UK_ground_temp.nc', decode_times=True) '''
...
class TempExtremes:
    ...
    def min_tot(self,save_as='min_tot.png'):
        ...
        qm = min_map.plot()
        ax = qm.axes
        ax.set_title(f"Overall minimum temperature values")
        ax.figure.savefig(save_as, bbox_inches='tight')
        plt.close(ax.figure)
    ...
    def max_tot(self,save_as='max_tot.png'):
        ...
        qm = max_map.plot()
        ax = qm.axes
        ax.set_title(f"Overall maximum temperature values")
        ax.figure.savefig(save_as, bbox_inches='tight')
        plt.close(ax.figure)
