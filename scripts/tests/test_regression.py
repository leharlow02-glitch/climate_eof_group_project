...
def test_make_yearly_resamples_correctly(tmp_path):
    ...
    # Because daily data = year value everywhere, yearly mean should equal that year
...
def test_regress_yearly_recovers_slope_and_masks(tmp_path):
    ...
    for v in ['slope', 'per_decade', 'p_value', 'r2', 'rmse', 'n_obs', 'intercept']:
    ...
    #Year 2001 to NaN for grid cell (lat=0, lon=0) so it has only 2 valid years
    times = ds2['time']
