...
class TempMean:
    ...
    def plot_mean_between(
        self, start, end, save_path="/root/climate_eof_group_project/plots"
    ):
        # Identify and returns the mean temperature values
        # between two dates and plots it to a map.
        ...
        ax.figure.savefig(
            save_path
            + f"/Mean_temperature_values_between_{start}_and_{end}.png",
            bbox_inches="tight",
        )
        plt.close(ax.figure)
    ...
    def plot_mean_tot_time(
        self, save_path="/root/climate_eof_group_project/plots/"
    ):
        ...
        ax.figure.savefig(
            save_path + "mean_time_tot_temperature.png", bbox_inches="tight"
        )
        plt.close(ax.figure)
    ...
    def plot_yearly_mean(
        self, out_dir="/root/climate_eof_group_project/plots/yearly_mean/"
    ):
        ...
        for idx, t in enumerate(times):
            ...
            plt.savefig(
                out_dir + f"mean {var_name} for {year_label}",
                dpi=150,
                bbox_inches="tight",
            )
            fig.tight_layout()
    ...
    def daily_clim(self, drop_feb29=True):
        ...
        If drop_feb29=True, Feb 29 is removed before computing the
        climatology so clim has 365 values.
        """
        daily = (
            self.tg
        )  # assume daily frequency; resample if needed:
        #self.tg.resample(time="1D").mean()
        if drop_feb29:
            daily_no29 = daily.sel(
                time=~(
    ...
    def daily_clim_Anom(self, keep_feb29=True, drop_feb29_for_clim=True):
        ...
        - If drop_feb29_for_clim=True, the climatology
            is computed on a 365-day calendar.
        - If keep_feb29=True, Feb 29 anomalies are produced
            by mapping Feb29 to Feb28 climatology.
        Otherwise Feb29 is dropped from the returned anomaly.
        Returns DataArray with same time axis as input
            (except possibly Feb29 if keep_feb29=False).
        """
        ...
        clim = daily_for_clim.groupby("time.dayofyear").mean(
            dim="time"
        )  # dayofyear: 1..365 (or 366)

        # Compute anomalies for days present in daily_for_clim
        # If we dropped Feb29 for climatology,
        # we compute anom for non-Feb29 days first
        anom_non29 = daily_for_clim.groupby("time.dayofyear") - clim

        if keep_feb29 and drop_feb29_for_clim:
            # Build Feb29 anomalies by mapping to Feb28
            # climatology (common choice)
            feb29_mask = (daily["time"].dt.month == 2) & (
                daily["time"].dt.day == 29
            )
            if feb29_mask.any():
                # dayofyear for Feb28 in non-leap-year calendar:
                feb28_dayofyear = pd.Timestamp(
                    year=2001, month=2, day=28
                ).dayofyear  # usually 59
                feb28_clim = clim.sel(dayofyear=feb28_dayofyear)

                # Compute anomaly for Feb29 dates by subtracting
                # Feb28 climatology
                feb29_values = daily.sel(time=feb29_mask) - feb28_clim

                # Combine non-Feb29 anomalies with constructed Feb29
                # anomalies and sort by time
                anom = xr.concat(
                    [anom_non29, feb29_values], dim="time"
                ).sortby("time")
            else:
                anom = anom_non29
        else:
            # either we dropped Feb29 and don't want to keep it,
            # or climatology included Feb29 so the grouping works
            # if daily still has Feb29 but clim has 366 days,
            #  xarray alignment will work automatically.
            if not drop_feb29_for_clim:
                # We didn't drop Feb29 when computing climatology,
                # so compute anom for full daily directly:
                anom = daily.groupby("time.dayofyear") - clim
            else:
                anom = anom_non29

        anom.attrs["description"] = (
            "Daily anomalies (daily - dayofyear climatology)"
        )
        return anom
    ...
    def plot_monthly_climatology(
        self, out_dir="/root/climate_eof_group_project/plots/monthly_clim/"
    ):
        ...
        for idx, t in enumerate(times):
            ...
            plt.savefig(
                out_dir + f"mean {var_name} for {month_label}",
                dpi=150,
                bbox_inches="tight",
            )
            fig.tight_layout()
